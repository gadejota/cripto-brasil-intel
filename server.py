"""
CRIPTO BRASIL INTEL — Backend v7
==================================
80 fontes RSS · Engine editorial v3 · Publisher · Scheduler
...
app = FastAPI(
    title="Cripto Brasil Intel API v7",
    description="80 fontes RSS · Engine editorial v3 · Viral score real · Carrosseis inteligentes",
    version="7.0.0",
)
app.add_middleware(
    CORSMiddleware,
...
# ─────────────────────────────────────────────────────────────────────────────
# 80 FONTES RSS — priorizando cripto PT-BR
# ─────────────────────────────────────────────────────────────────────────────
FEEDS = [
    # ── CRIPTO BR TIER 1 — alta prioridade ───────────────────────────────────
    ...
    {"url":"https://www.moneytimes.com.br/feed/",
     "name":"MoneyTimes","src":"t-mt","cat":"cripto","br":True,"weight":1.5},
    ...
    {"url":"https://coinext.com.br/blog/feed",
     "name":"Coinext Blog","src":"t-cx","cat":"cripto","br":True,"weight":1.3},

    # ── CRIPTO BR NOVAS FONTES ────────────────────────────────────────────────
    {"url":"https://blog.mercadobitcoin.com.br/feed",
     "name":"Mercado Bitcoin Blog","src":"t-mb","cat":"cripto","br":True,"weight":1.8},
    {"url":"https://hashdex.com/pt-BR/blog/feed",
     "name":"Hashdex Blog","src":"t-hx","cat":"cripto","br":True,"weight":1.7},
    {"url":"https://foxbit.com.br/blog/feed/",
     "name":"Foxbit Blog","src":"t-fb","cat":"cripto","br":True,"weight":1.5},
    {"url":"https://novadax.com/blog/feed",
     "name":"NovaDax Blog","src":"t-nd","cat":"cripto","br":True,"weight":1.3},
    {"url":"https://blog.bitget.com/pt/feed",
     "name":"Bitget Blog BR","src":"t-bg","cat":"cripto","br":True,"weight":1.3},
    {"url":"https://www.bitcointrade.com.br/blog/feed/",
     "name":"BitcoinTrade Blog","src":"t-bt","cat":"cripto","br":True,"weight":1.3},
    {"url":"https://www.cryptoradar.com.br/feed/",
     "name":"CryptoRadar BR","src":"t-cr","cat":"cripto","br":True,"weight":1.2},
    {"url":"https://www.criptointeligente.com.br/feed/",
     "name":"Cripto Inteligente","src":"t-ci","cat":"cripto","br":True,"weight":1.3},

    # ── CRIPTO GLOBAL TIER 1 ──────────────────────────────────────────────────
    {"url":"https://www.coindesk.com/arc/outboundfeeds/rss/",
     "name":"CoinDesk","src":"t-cd","cat":"cripto","br":False,"weight":1.8},
    {"url":"https://cointelegraph.com/rss",
     "name":"Cointelegraph","src":"t-ct","cat":"cripto","br":False,"weight":1.7},
    {"url":"https://www.theblock.co/rss.xml",
     "name":"The Block","src":"t-tb","cat":"cripto","br":False,"weight":1.6},
    {"url":"https://bitcoinmagazine.com/.rss/full/",
     "name":"Bitcoin Magazine","src":"t-bm","cat":"cripto","br":False,"weight":1.7},
    {"url":"https://decrypt.co/feed",
     "name":"Decrypt","src":"t-dc","cat":"cripto","br":False,"weight":1.5},

    # ── CRIPTO GLOBAL TIER 2 ──────────────────────────────────────────────────
    {"url":"https://blockworks.co/feed",
     "name":"Blockworks","src":"t-bk","cat":"cripto","br":False,"weight":1.5},
    {"url":"https://thedefiant.io/feed",
     "name":"The Defiant","src":"t-def","cat":"cripto","br":False,"weight":1.4},
    {"url":"https://unchainedcrypto.com/feed/",
     "name":"Unchained Crypto","src":"t-uc","cat":"cripto","br":False,"weight":1.4},
    {"url":"https://coinmarketcap.com/rss/news.xml",
     "name":"CoinMarketCap News","src":"t-cmc","cat":"cripto","br":False,"weight":1.5},
    {"url":"https://dlnews.com/rss.xml",
     "name":"DL News","src":"t-dln","cat":"cripto","br":False,"weight":1.4},
    {"url":"https://www.cryptoslate.com/feed/",
     "name":"CryptoSlate","src":"t-cs","cat":"cripto","br":False,"weight":1.3},
    {"url":"https://cryptobriefing.com/feed/",
     "name":"Crypto Briefing","src":"t-cb","cat":"cripto","br":False,"weight":1.2},
    ...
    {"url":"https://beincrypto.com/feed/",
     "name":"BeInCrypto","src":"t-bic2","cat":"cripto","br":False,"weight":1.3},
    {"url":"https://cryptonews.com/news/feed/",
     "name":"CryptoNews","src":"t-cn","cat":"cripto","br":False,"weight":1.2},
    {"url":"https://www.newsbtc.com/feed/",
     "name":"NewsBTC","src":"t-nb","cat":"cripto","br":False,"weight":1.2},
    ...
    {"url":"https://www.coinbureau.com/feed/",
     "name":"Coin Bureau","src":"t-cbu","cat":"cripto","br":False,"weight":1.3},
...
# ─────────────────────────────────────────────────────────────────────────────
# KEYWORDS — ponderadas por relevância cripto
# ─────────────────────────────────────────────────────────────────────────────
# Tier A: palavras de alta viralidade cripto (peso 6 cada)
CRYPTO_KW_A = [
    "bitcoin","btc","halving","etf bitcoin","saylor","microstrategy","strategy",
    "blackrock bitcoin","fidelity bitcoin","spot etf","lightning network",
    "bitcoin treasury","bitcoin reserve","satoshi",
]
# Tier B: cripto geral (peso 4 cada)
CRYPTO_KW_B = [
    "ethereum","eth","solana","xrp","ripple","binance","coinbase","defi",
    "stablecoin","nft","web3","altcoin","blockchain","on-chain","onchain",
    "ordinals","rune","layer2","dao","yield","liquidity","exchange cripto",
    "carteira crypto","wallet cripto","cripto","crypto",
]
# Tier C: termos de contexto (peso 2 cada)
CRYPTO_KW_C = [
    "token","mining","mineracao","mvrv","hash rate","futures cripto",
    "derivative","protocol","hodl","bull market","bear market",
]
...
GEO_KW = [
    ...
    "pentagon","kremlin","zelensky","putin","trump","china","taiwan",
    "north korea","coreia","middle east","oriente medio","coup","golpe","drone",
    "election","eleicao","submarine","hypersonic","strait","oil tanker",
]

# ─────────────────────────────────────────────────────────────────────────────
# PALAVRAS QUE AUMENTAM VIRALIDADE
# ─────────────────────────────────────────────────────────────────────────────
VIRAL_WORDS = [
    # Urgência/breaking
    "breaking","urgente","alerta","record","recorde","all-time","ath","historic",
    "histórico","crash","rompe","despenca","explode","derrete","dispara",
    # Números grandes (se aparecerem junto com cripto)
    "bilhão","billion","trilhão","trillion","milhões","million",
    # Institucional
    "blackrock","fidelity","jpmorgan","goldman","ark","hashdex","grayscale",
    "tesla","microstrategy","strategy","saylor",
    # Regulação BR
    "receita federal","cvm","bacen","banco central","regulação","iof",
    # Movimentos extremos
    "100k","$100","$200","200k","1 milhão","1 million","nova máxima","new high",
]

BORING_WORDS = [
    "podcast","interview","entrevista","opinion","opinião","weekly","semanal",
    "digest","newsletter","roundup","recap","summary","resumo","análise técnica",
]
...
async def fetch_feed(client: httpx.AsyncClient, feed: dict) -> list:
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; CriptoBrasilIntel/7.0; RSS reader)",
...
# ─────────────────────────────────────────────────────────────────────────────
# VIRAL SCORE — novo sistema, 0-200
# ─────────────────────────────────────────────────────────────────────────────
def score_article(a: dict) -> int:
    ...
    # ── 1. BASE DA FONTE (0–40) ───────────────────────────────────────────────
    # Fontes de qualidade que geram mais engajamento
    score += int(w * 20)  # max ~40 para weight=2.0

    # ── 2. RECÊNCIA — quando saiu é crítico (0–40) ────────────────────────────
    ...
    try:
        ...
        if age_h <= 1:    score += 40   # menos de 1h — muito quente
        elif age_h <= 3:  score += 32
        elif age_h <= 6:  score += 25
        elif age_h <= 12: score += 18
        elif age_h <= 24: score += 10
        elif age_h <= 48: score += 4
        # mais de 48h: +0
    except:
        # Fallback: verifica texto da data
        today = now.strftime("%d %b %Y")
        if today in pub:
            score += 12

    # ── 3. RELEVÂNCIA CRIPTO — tiered keywords (0–45) ────────────────────────
    for kw in CRYPTO_KW_A:
        if kw in text:
            score += 6
    for kw in CRYPTO_KW_B:
        if kw in text:
            score += 4
    for kw in CRYPTO_KW_C:
        if kw in text:
            score += 2

    # Cap keywords cripto em 45
    # (se tiver 10+ keywords vai estourar, limitamos no final)

    # ── 4. BR CRIPTO — bônus estratégico (0–20) ──────────────────────────────
    if br and cat == "cripto":
        score += 20   # fonte BR + categoria cripto = conteúdo perfeito pro canal
    elif br:
        score += 10   # BR mas não cripto
    elif cat == "cripto":
        score += 5    # cripto mas não BR

    # ── 5. PALAVRAS DE VIRALIDADE (0–30) ─────────────────────────────────────
    for vw in VIRAL_WORDS:
        if vw in text:
            score += 5
    # Cap 30

    # ── 6. NÚMEROS NO TÍTULO — especificidade gera clique (0–15) ─────────────
    numbers = re.findall(
        r"(?:us?\$|r\$|\$|€)\s*[\d,\.]+[kKmMbBtT]?|[\d,\.]+\s*%|[\d,\.]+\s*(?:mil|bilh|trilh)",
        title.lower()
    )
    ...
    # Títulos em inglês puro numa fonte BR têm menos engajamento
    if br and not any(c in text for c in ["ã","ç","ê","ó","á","é","í","ú","ô","â"]):
        score -= 5
...
def classify_content(a: dict) -> str:
    ...
    bull_kw = ["alta","subiu","sobe","atingiu","supera","recorde","bullish",
               "surge","rally","all-time","ath","soars","rises","gains",
               "comprou","bought","buys","adquiriu","new high","nova maxima"]
    bear_kw = ["queda","caiu","cai","despenca","colapso","crash","pánico","panico",
               "bearish","falls","drops","plunge","selloff","decline","correction",
               "despencou","derreteu","broke down","rompeu suporte"]
    edu_kw  = ["como","por que","o que é","guia","entenda","aprenda","explica",
               "tutorial","what is","how to","why","explained","guide","saiba",
               "descubra","understand","for beginners","para iniciantes"]
...
async def build_news(n_cripto=14, n_macro=8, n_geo=6) -> list:
    ...
    # Prioriza cripto BR no topo da seleção cripto
    cripto_br     = [a for a in all_articles if a["cat"] == "cripto" and a["br"]][:n_cripto//2 + 2]
    cripto_global = [a for a in all_articles if a["cat"] == "cripto" and not a["br"]][:n_cripto//2]
    cripto_ids    = {id(a) for a in cripto_br + cripto_global}
    cripto        = (cripto_br + cripto_global)[:n_cripto]
    ...
    for i, a in enumerate(selected[:MAX_ART]):
        a["id"] = i + 1
        # Aliases para editorial_engine
        a["_cls"]        = a.get("cls", "edu")
    ...
    cripto_count = sum(1 for a in enriched if a.get("cat") == "cripto")
    br_count     = sum(1 for a in enriched if a.get("br"))
    log.info(f"Done: {len(enriched)} artigos | {cripto_count} cripto | {br_count} BR")
    return enriched
...
async def startup():
    log.info(f"Cripto Brasil Intel v7 | {len(FEEDS)} fontes | porta {PORT}")
...
if __name__ == "__main__":
    cripto_br_count = sum(1 for f in FEEDS if f["br"] and f["cat"] == "cripto")
    print(f"\n Cripto Brasil Intel v7")
    print(f"   {len(FEEDS)} fontes RSS | {cripto_br_count} cripto BR")
    print(f"   {len(FORMATS)} formatos editoriais | Viral score v3")
